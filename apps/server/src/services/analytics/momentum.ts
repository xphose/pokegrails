...
import { loadAllPriceHistory, mean, stddev, recordModelRun, linearRegression } from './shared.js'
...
export function getCardMomentum(
  ...
  const all = detectMomentumCards(db, 5000)
  const found = all.find(c => c.card_id === cardId)
